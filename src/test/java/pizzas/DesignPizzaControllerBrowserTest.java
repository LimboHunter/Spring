...
public class DesignPizzaControllerBrowserTest {
  ...
  public void testDesignATacoPage() throws Exception {
    ...
    WebElement crustGroup = ingredientGroups.get(0);
    List<WebElement> wraps = crustGroup.findElements(By.tagName("div"));
    assertThat(wraps).hasSize(2);
    assertIngredient(crustGroup, 0, "FLTO", "Flour Tortilla");
    assertIngredient(crustGroup, 1, "COTO", "Corn Tortilla");
    
    WebElement meatGroup = ingredientGroups.get(1);
    List<WebElement> proteins = meatGroup.findElements(By.tagName("div"));
    assertThat(proteins).hasSize(2);
    assertIngredient(meatGroup, 0, "GRBF", "Ground Beef");
    assertIngredient(meatGroup, 1, "CARN", "Carnitas");
  }
